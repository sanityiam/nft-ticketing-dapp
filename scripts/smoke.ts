...
async function main() {
  ...
  // create event
  const tx = await platformOrg.createEvent(
    "Dubai Concert",
    "Coca-Cola Arena",
    dateTime,
    basePrice,
  ...
  // Find EventCreated in receipt logs
  const log = rcpt!.logs.find((l: any) => l.fragment?.name === "EventCreated");
  if (!log || !("args" in log)) throw new Error("EventCreated not found in receipt");

  const eventId = (log as any).args.eventId as bigint;
